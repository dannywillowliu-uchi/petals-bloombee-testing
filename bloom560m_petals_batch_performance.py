#!/usr/bin/env python3
"""
Performance test script for BLOOM-560m model via Petals over ngrok tunnel
Connects to Colab server running BLOOM-560m
"""
...
NGROK_HOST = "3.141.142.211"
NGROK_PORT = 17423
SERVER_PEER_ID = "12D3KooWSpgTubtWBfWQz2Zr5brMe2iFgWPjYP4vMTe9TpfrwZC4"
...
async def test_single_inference():
    ...
    # Connect to the private swarm
    initial_peers = [f"/ip4/{NGROK_HOST}/tcp/{NGROK_PORT}/p2p/{SERVER_PEER_ID}"]
    
    print(f"Connecting to private server: {initial_peers[0]}")
...
async def main():
    print("Starting BLOOM-560m Petals Performance Test")
    print(f"Connecting to ngrok tunnel: {NGROK_HOST}:{NGROK_PORT}")
    print(f"Server peer ID: {SERVER_PEER_ID}")
    print(f"Model: {MODEL_NAME}")
    print(f"DHT prefix: {DHT_PREFIX}")
    print("=" * 60)
    ...
    plt.title('BLOOM-560m Petals Inference Performance')
    plt.xticks(batch_sizes)
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig('bloom560m_petals_batch_performance.png', dpi=300, bbox_inches='tight')
    print(f"\nPerformance graph saved as 'bloom560m_petals_batch_performance.png'")
